...
#cuda imports for basic information about device
...
class HWAnalysis(object):
    ...
    def comb_base_xor_GPU(self, top_comb_cur, top_terms, top_res, num_evals, poly_builder, ref_hws=None):
        """
        Base skeleton for generating all XOR combinations from top_terms to degree top_comb_cur
        with help of GPU. For documentation see @comb_base
        """
        #print("XOR COMB BASE GPU")
        polynoms = []
        expps = []
        exp_cnts = []
        for idx, places in enumerate(common.term_generator(top_comb_cur, len(top_terms) - 1, self.prob_comb)):
        ...
        #for i in range(0,numPolys):
        #    print(polynoms[i])        
        obs_cnts = self.term_eval.calculate_xor_obs_counts(top_comb_cur,len(top_terms),polynoms,termLen,polyLen,numPolys)
        #for i in range(0,numPolys):
        #    print("poly",polynoms[i],"count",obs_cnts[i])
    ...
    def comb_base(self, top_comb_cur, top_terms, top_res, num_evals, poly_builder, ref_hws=None):
        ...
        overall = 0
        overall2 = 0
        #problemom je term_eval.hw(term_eval.eval_poly....)
        for idx, places in enumerate(common.term_generator(top_comb_cur, len(top_terms) - 1, self.prob_comb)):
            poly = poly_builder(places, top_terms)
            expp = self.term_eval.expp_poly(poly)
            exp_cnt = num_evals * expp
            ...
            self.comb_add_result(comb, top_res)
        #print("term_eval.hw z comb_base celkovo:", overall)
        #print("zscore z comb_base celkovo:", overall2)
    ...
    def comb_xor(self, top_comb_cur, top_terms, top_res, num_evals, ref_hws=None):
        ...
        """
        start = time.time()
        self.comb_base_xor_GPU(top_comb_cur, top_terms, top_res, num_evals, poly_builder, ref_hws)
        end = time.time()
        print("GPU odhad XOR",end-start)
        
        start= time.time()
        self.comb_base(top_comb_cur, top_terms, top_res, num_evals, poly_builder, ref_hws)
        end = time.time()
        print("CPU odhad XOR",end-start)
        """
        if self.compute_on_cpu:
            ...
            #print("XOR")
            #self.comb_base(top_comb_cur, top_terms, top_res, num_evals, poly_builder, ref_hws)
            return self.comb_base_xor_GPU(top_comb_cur, top_terms, top_res, num_evals, poly_builder, ref_hws)
    ...
    def comb_and(self, top_comb_cur, top_terms, top_res, num_evals, ref_hws=None):
        ...
        """
        start = time.time()
        self.comb_base_and_GPU(top_comb_cur, top_terms, top_res, num_evals, poly_builder, ref_hws)
        end = time.time()
        print("GPU odhad AND",end-start)
    
        start= time.time()
        self.comb_base(top_comb_cur, top_terms, top_res, num_evals, poly_builder, ref_hws)
        end = time.time()
        print("CPU odhad AND",end-start)
        """
        if self.compute_on_cpu:
            return self.comb_base(top_comb_cur, top_terms, top_res, num_evals, poly_builder, ref_hws)
        else:
            #print("AND")
            #self.comb_base(top_comb_cur, top_terms, top_res, num_evals, poly_builder, ref_hws)
            return self.comb_base_and_GPU(top_comb_cur, top_terms, top_res, num_evals, poly_builder, ref_hws)
...
class Booltest(object):
    ...
    def main(self):
        ...
        parser.add_argument('--compute-on-cpu', dest='compute_on_cpu', action='store_const', const=True, default=False,
                            help='Compute data on CPU')
        
        parser.add_argument('--verify', dest='verify', action='store_const', const=True, default=False,
                            help='Verify computed GPU data with CPU computation. '
                                 'If data are not same AssertErrorException is raised')
