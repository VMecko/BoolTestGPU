...
"""
This is a starting point for adding any function which will
be run on GPU. We implement functionality here and then call
it through python functions. Write code in C or C++. 
"""
mod = SourceModule("""
	#include <stdio.h>
        #include <stdlib.h>
        #include <inttypes.h>
        __device__ __constant__ unsigned int numOfANDsConst[1];
        __device__ __constant__ unsigned int XConst[1];
        __device__ __constant__ int degConst[1];
        __device__ __constant__ int sliceConst[1];
	
	__global__ void countOnes(int64_t* in, unsigned int* result) {
              unsigned int id = blockIdx.x * blockDim.x * blockDim.y + threadIdx.y * blockDim.x + threadIdx.x;
                int numOfOnes = __popcll(in[id]);
                atomicAdd(&result[(int)(id/XConst[0])],(unsigned int)numOfOnes);
	}
        
        
        __global__ void countAND(int64_t* in,unsigned int* result, int* twoBAs){
              unsigned int id = blockIdx.x * blockDim.x * blockDim.y + threadIdx.y * blockDim.x + threadIdx.x;
              if(id < numOfANDsConst[0]){
                 int first = twoBAs[2*id];
                 int second = twoBAs[(2*id) + 1];
                 for(unsigned int i = 0; i<XConst[0];i++){
                     int64_t temp = in[(XConst[0] * first) + i] & in[(XConst[0] * second) + i];
                     int numOfOnes = __popcll(temp);
                     atomicAdd(&result[id],(unsigned int) numOfOnes);
                 }
              }
        }
        
        __global__ void genCountConstSimple(int64_t* in, unsigned int* result, int* BAs){
               const int max_degree = 6;
               unsigned int id = blockIdx.x * blockDim.x * blockDim.y + threadIdx.y * blockDim.x + threadIdx.x;
               if(id < (numOfANDsConst[0] * XConst[0])){
                 
                 int indices[max_degree];
                 unsigned int y = id / XConst[0];
                 for(int i = 0; i < degConst[0];i++){
                   indices[i] = BAs[(degConst[0] * y) + i];
                 }
                 
                 int64_t tmp = in[(XConst[0] * indices[0]) + (id % XConst[0])];
                 for(int j = 1; j < degConst[0];j++){
                   tmp = tmp & in[(XConst[0] * indices[j]) + (id % XConst[0])];
                 }
                 int numOfOnes = __popcll(tmp);
                 atomicAdd(&result[y],(unsigned int) numOfOnes);
                 //printf("%d:koncim\\n",id);
            }
        }
        
        __global__ void genCountWithConstDegThree(int64_t* in, unsigned int* result, int* BAs){
               unsigned int id = blockIdx.x * blockDim.x * blockDim.y + threadIdx.y * blockDim.x + threadIdx.x;
               if(id < numOfANDsConst[0]){
                  /* indices of position of bits from bitarray block 
                   * be careful because first value is on position indices(0).
                   * Also we cant dynamically allocate degree during run (because its a CUDA nvcc compiler code)
                   * and the memory wont be saved into local space (but into global)
                   */
                   int indices[3];
                   for(int i = 0; i<degConst[0];i++){
                      indices[i] = BAs[(degConst[0] * id) + i];
                   }
                   for(long int i = 0; i<XConst[0];i++){
                        register int64_t temp = in[(XConst[0] * indices[0]) + i];
                            for(int j = 1; j<degConst[0];j++){
                            temp = temp & in[(XConst[0] * indices[j]) + i];
                            }
                        int numOfOnes = __popcll(temp);
                        atomicAdd(&result[id],(unsigned int) numOfOnes);
                   }
        ...
        __global__ void genCountWithConstDegTwo(int64_t* in, unsigned int* result, int* BAs){
               unsigned int id = blockIdx.x * blockDim.x * blockDim.y + threadIdx.y * blockDim.x + threadIdx.x;
               if(id < numOfANDsConst[0]){
                  /* indices of position of bits from bitarray block 
                   * be careful because first value is on position indices(0).
                   ...
                   */
                   ...
                   for(long int i = 0; i<XConst[0];i++){
                        register int64_t temp = in[(XConst[0] * indices[0]) + i] & in[(XConst[0] * indices[1]) + i];
                        int numOfOnes = __popcll(temp);
                        atomicAdd(&result[id],(unsigned int) numOfOnes);
                   }
                }
        ...
        __global__ void generalCountAND(int64_t* in, unsigned int* result, int* BAs){
               unsigned int id = blockIdx.x * blockDim.x * blockDim.y + threadIdx.y * blockDim.x + threadIdx.x;
               if(id < numOfANDsConst[0]){
                   const int max_degree = 6;
                  /* indices of position of bits from bitarray block 
                   * be careful because first value is on position indices(0).
                   * Also we cant dynamically allocate degree during run (because its a CUDA nvcc compiler code)
                   * so i set max_degree to 6 even though , not all indices needs to be used.
                   */
                   int indices[max_degree];
                   for(int i = 0; i<degConst[0];i++){
                   indices[i] = BAs[(degConst[0] * id) + i];
                        ...
                        /* each row of i-th bits can use multiple 64bits variables since
                         * we store them as 64-bit integers. We need to go through whole
                         * array.
                         */
                        for(unsigned int i = 0; i<XConst[0];i++){
                                int64_t temp = in[(XConst[0] * indices[0]) + i];
                                //we AND all indices together
                                for(int j = 1; j<degConst[0];j++){
                                  temp = temp & in[(XConst[0] * indices[j]) + i];
                                }
                                int numOfOnes = __popcll(temp);
         ...
         /**
           * Just a little comment here about "clearance" of code.
           * Yes, i can store the indices into variables then call
           * BAs[index1], BAs[index2],... and then in for cycle 
           * & them all with some tmp subresult but that's not
           * effective on GPU. We want to reduce memory movement
           * so i just let the threads to calculate indices everytime
           * since its faster. in general it's: (3*slice*x) + (0|1|2) + (i*3)
           * meaning: we do slice*3 numbers from BA in one thread X, so we skip them
           * then we use the next three indices + we skip the already processes i*3 
           * BAs for this thread x.
           */
        __global__ void genCountShMem(int64_t* in, unsigned int* result,int* BAs){
                int x = threadIdx.x;
                int y = threadIdx.y;
                unsigned int dimx = blockDim.x;
                unsigned int dimy = blockDim.y;
                __shared__ int64_t chunk[1024]; //128/1024
                //predtym pre 128 blockIdx.x + x*XConst[0],1024 (dimy*blockIdx.x) + (x * XConst[0]) + y
                chunk[(dimx*y)+x] = in[(dimy*blockIdx.x) + (x * XConst[0]) + y]; //chunk[(dimx*y) + x]
                __syncthreads();
                for(unsigned int i = 0; i < sliceConst[0];i++){
                        //predtym pre 128 3*x*sliceConst + i*3 + 0|1|2
                        int index1 = BAs[3*x*sliceConst[0] + i*3];
                        int index2 = BAs[3*x*sliceConst[0] + i*3 + 1];
                        int index3 = BAs[3*x*sliceConst[0] + i*3 + 2];
                        //printf("%d %d %d %d\\n",x,3*x*sliceConst[0] + i*3,sliceConst[0],(sliceConst[0]*x)+i);
                        int64_t temp = chunk[index1+y*dimx] & chunk[index2+y*dimx] & chunk[index3+y*dimx]; //chunk[index1+y*dimx]
                        //temp = temp & chunk[index3];
                        //int64_t temp = in[index1] & in[index2] & in[index3];
                        int numOfOnes = __popcll(temp);
                        atomicAdd(&result[(sliceConst[0]*x) + i],(unsigned int) numOfOnes);
                }
        }
        
        __global__ void calculateAndTermsInPoly(int64_t* in,int64_t* subresult,unsigned int* result, int* terms, int termLen, int numTerms, int varsInBlock){
           unsigned int id = blockIdx.x * blockDim.x * blockDim.y + threadIdx.y * blockDim.x + threadIdx.x;
           /**
             * kazde id vypocita indexy svojich termov
             * potom spravi & vsetkych 64bit blokov svojich termov
             * to je vse v prvej casti, v druhej(pod touto metodou) XOR/AND a count
             */
            if(id < numTerms){
                int termNumbers[4];
                for(unsigned int i = 0; i < termLen;i++){
                  termNumbers[i] = terms[(id*termLen)+i];
                }
                //printf("%d: %d %d %d\\n",id, termNumbers[0],termNumbers[1],termNumbers[2]);
                for(unsigned int i = 0; i < varsInBlock; i++){
                  int64_t temp = in[(varsInBlock * termNumbers[0]) + i];
                  for(unsigned int j = 1; j < termLen;j++){
                     temp = temp & in[(varsInBlock * termNumbers[j]) + i];
                  }
                  subresult[(id*varsInBlock)+i] = temp;
                  int numOfOnes = __popcll(temp);
                  atomicAdd(&result[id],(unsigned int) numOfOnes);
                }
        ...
        __global__ void calcXORcombinations(int64_t* in,int* combinations,int* result,int polyLen, int numOfPolys,int varsInBlock){
         int id = blockIdx.x * blockDim.x * blockDim.y + threadIdx.y * blockDim.x + threadIdx.x;
         if(id < numOfPolys){
           int combs[5];
           for(int i = 0; i < polyLen; i++){
               combs[i] = combinations[(polyLen * id) + i];
           }
           //int first = combinations[2*id];
           //int second = combinations[2*id + 1];
           //printf("%d: %d %d\\n",id,first,second);
           for(long int i = 0; i < varsInBlock; i++){
            int64_t temp = in[varsInBlock*combs[0] + i];
            for(int j = 1; j < polyLen; j++){
                 temp = temp ^ in[varsInBlock*combs[j] + i];
            }
            int numOfOnes = __popcll(temp);
            atomicAdd(&result[id],(unsigned int) numOfOnes);
           }
      ...
      __global__ void calcANDcombinations(int64_t* in, int* combinations, int* result,int polyLen, int numOfPolys, int varsInBlock){
       int id = blockIdx.x * blockDim.x * blockDim.y + threadIdx.y * blockDim.x + threadIdx.x;
         if(id < numOfPolys){
           int combs[5];
           for(int i = 0; i < polyLen; i++){
...
#this way we link python variables to C CUDA functions
countOnes = mod.get_function("countOnes")
countAND = mod.get_function("countAND")
generalCountAND = mod.get_function("generalCountAND")
genCountWithConstTwo = mod.get_function("genCountWithConstDegTwo")
genCountWithConstThree = mod.get_function("genCountWithConstDegThree")
genCountConstSimple = mod.get_function("genCountConstSimple")
genCountShMem = mod.get_function("genCountShMem")
calculateAndTermsInPoly = mod.get_function("calculateAndTermsInPoly")
calculateXORcombinations = mod.get_function("calcXORcombinations")
calculateANDcombinations = mod.get_function("calcANDcombinations")

#this way we link python variables to C CUDA constants
numANDsConst = mod.get_global("numOfANDsConst")[0]
XCons = mod.get_global("XConst")[0] 
degCons = mod.get_global("degConst")[0]
sliceCons = mod.get_global("sliceConst")[0]
...
def generateCombinations(num_items,deg):
    """
    returns list (not a numpy array)
    """
    lst = range(0,num_items)
    return list(combinations(lst,deg))

def adjust_size64(length):
    """
    Calculates size of alignment, which is needed to add, to adjust block of data to match 64-bit size of variable.
    This feature is mainly used in function eval_all_terms to adjust data for GPU calculations.
        
    :param length: current length of object
...
class TermEval(object):
    ...
    def get_max_threads(self):
        """
        Returns maximum of threads per block used on computing device (GPU).
        :return:
        """
        device = cuda.Device(0)
        
        #by default we set max_threads to 512 (CUDA compute capability < 2.0)
        max_threads = 512
    ...
    def eval_all_terms_GPU(self, deg=None):
        """
        Evaluates all terms of deg [1, deg].
        Evaluation is done on GPU.
        
        :warning: Works only with CUDA capable GPU.
        :param deg:
        :return:
        """         
        if deg is None:
        ...
        mergedBArr = bitarray.bitarray()
        alignment = adjust_size64(self.base[0].length())
        varsInBlock = (self.base[0].length() + alignment) / 64
        for i in range(0,len(self.base)):
            mergedBArr += self.base[i]
            mergedBArr += bitarray.bitarray('0' * alignment)
            
        #CPU alloc
        block = np.array([varsInBlock],dtype = np.uint32)
        #print("varsinblock",block[0], len(self.base))
        result = np.zeros(len(self.base),dtype = np.uint32)
        resultDegOne = np.empty_like(result)
        
        #GPU alloc
        result_GPU = cuda.mem_alloc(result.nbytes)
        bitarray_GPU = cuda.mem_alloc(mergedBArr.length()/8)
        
        #CPU to GPU
        cuda.memcpy_htod(result_GPU,result)
        cuda.memcpy_htod(bitarray_GPU, mergedBArr)
        #const
        cuda.memcpy_htod(XCons,block) 
        start = cuda.Event()
        end = cuda.Event()
        
        start.record()
        if max_threads == 1024 :
            countOnes(bitarray_GPU, result_GPU, block=(32,32,1), grid=((self.blocklen*self.base[0].length())/(32*32*64),1))
        else:
            countOnes(bitarray_GPU, result_GPU, block=(32,16,1), grid=((self.blocklen*self.base[0].length())/(32*16*64),1))
        end.record()
        end.synchronize()
        secs = start.time_till(end)*1e-3
        print("countOnes",secs)
        #GPU to CPU
        cuda.memcpy_dtoh(resultDegOne, result_GPU)
        hw[1] = resultDegOne
        ...
        if deg == 2:
            #CPU alloc
            num_terms = self.num_terms(2,False,exact=True)
            hw[2] = [0] * num_terms    
            resultDegTwo = np.zeros(num_terms,dtype=np.uint32)
            numOfANDs = np.array([num_terms],dtype= np.uint32) 
            # npPairs is array for storing all pairs of combinations of terms
            # we don't want to calculate the pairs on CPU since that takes time
            # so we precalculate the pairs and then each thread calculates exactly one pair
            npPairs = np.empty(2 * num_terms, dtype = np.uint32)
            for idx, term in enumerate(self.term_generator(2)):
                npPairs[2 * idx] = term[0]
                npPairs[(2 * idx) + 1] = term[1]
            
            #GPU alloc
            npPairs_GPU = cuda.mem_alloc(npPairs.nbytes)
            resultDegTwo_GPU = cuda.mem_alloc(resultDegTwo.nbytes)
            
            #CPU to GPU
            cuda.memcpy_htod(npPairs_GPU,npPairs)
            cuda.memcpy_htod(resultDegTwo_GPU,resultDegTwo)
            #const
            cuda.memcpy_htod(numANDsConst,numOfANDs)  
            cuda.memcpy_htod(XCons,block)
            """
            start = cuda.Event()
            end = cuda.Event()
            start.record()
            countAND(bitarray_GPU,resultDegTwo_GPU,npPairs_GPU,block=(64,1,1),grid=((num_terms/64)+1,1))            
            end.record()
            end.synchronize()
            secs = start.time_till(end)*1e-3            
            print("algo 2.1",secs)
            ...
            start.record()
            countAND(bitarray_GPU,resultDegTwo_GPU,npPairs_GPU,block=(max_threads,1,1),grid=((num_terms/max_threads)+1,1))            
            end.record()
            end.synchronize()
            secs = start.time_till(end)*1e-3
            print("algo 2.2",secs)
            #GPU to CPU
            cuda.memcpy_dtoh(resultDegTwo,resultDegTwo_GPU)
            hw[2] = resultDegTwo
        ...
        for i in range(2,deg+1):
            #CPU alloc
            num_terms = self.num_terms(i,False,exact=True)
            npDeg = np.array([i],dtype = np.uint32)
            npPairing = np.empty(i * num_terms, dtype = np.uint32)
            for idx,term in enumerate(self.term_generator(i)):
                for j in range(0,len(term)):
                    npPairing[(i * idx) + j] = term[j]            
            
            #CPU to GPU 
            #const
            cuda.memcpy_htod(degCons,npDeg)                
            
            # since degree > 4 take quite a lot of memory to calculate, the main idea behind this is to
            # split calculation into multiple slices -> that way we can calculate with bigger data
            # and don't need to worry about npPairing usage of memory since comb(block,deg) * deg * 4B is a lot of memory used. 
            if(i>4):
                # set this to whatever you want but more slices = bigger computing time 
                # because of overhead created with each execution of kernel(GPU) method
                slices = 10
                npPairingSlice = num_terms / slices #1/slices part of npPairing (don't forget about the current degree (so slice is of size (num_terms/4) * i)
                genCountResult = np.empty(npPairingSlice,dtype=np.uint32)
                resultGenDeg = np.zeros(npPairingSlice,dtype=np.uint32)
                
                resultGenDeg_GPU = cuda.mem_alloc(resultGenDeg.nbytes)
                
                for j in range(0,slices-1):
                    numOfAND = np.array([npPairingSlice],dtype = np.uint32)
                                    
                    npPairingSlice_GPU = cuda.mem_alloc(npPairingSlice * i * 4) #because 4B is size of uint32, we need i numbers for degree i
                    
                    cuda.memcpy_htod(resultGenDeg_GPU,resultGenDeg)                    
                    #print("indexy pre j:",j, j*i*npPairingSlice,((j+1) * i * (npPairingSlice))- 3)
                    cuda.memcpy_htod(npPairingSlice_GPU,npPairing[(j*i*npPairingSlice):((j+1) * i * npPairingSlice)])
                    cuda.memcpy_htod(numANDsConst,numOfAND)
                    cuda.memcpy_htod(XCons,block)
                    
                    generalCountAND(bitarray_GPU,resultGenDeg_GPU,varsInBlock_GPU,npPairingSlice_GPU,block=(512,2,1),grid=(100000,1))
                    if(j == 0):
                        cuda.memcpy_dtoh(genCountResult,resultGenDeg_GPU)                        
                    else:
                        genCountSliceResult = np.empty_like(resultGenDeg)
                        cuda.memcpy_dtoh(genCountSliceResult,resultGenDeg_GPU)
                        genCountResult = np.concatenate((genCountResult,genCountSliceResult), axis=0)
                #now the final part of npPairing (might be different size as npPairingSlice)
                lastPairingSlice = num_terms - ((slices-1) * npPairingSlice)
                numOfAND = np.array([lastPairingSlice],dtype= np.uint32)
                resultGenDeg = np.zeros(lastPairingSlice,dtype=np.uint32)
                
                resultGenDeg_GPU = cuda.mem_alloc(resultGenDeg.nbytes)
                npPairingSlice_GPU = cuda.mem_alloc(lastPairingSlice * i * 4) #because 4B is size of uint32, we need i numbers for degree i
                
                cuda.memcpy_htod(resultGenDeg_GPU,resultGenDeg)
                cuda.memcpy_htod(npPairingSlice_GPU,npPairing[((slices-1)*i*npPairingSlice):npPairing.size])
                cuda.memcpy_htod(numANDsConst,numOfAND)
                
                generalCountAND(bitarray_GPU,resultGenDeg_GPU,npPairingSlice_GPU,block=(128,1,1),grid=(100000,1))
                genCountSliceResult = np.empty_like(resultGenDeg)
                cuda.memcpy_dtoh(genCountSliceResult,resultGenDeg_GPU)
                genCountResult = np.concatenate((genCountResult,genCountSliceResult),axis=0)
            else:
                resultGenDeg = np.zeros(num_terms,dtype=np.uint32)
                numOfAND = np.array([num_terms],dtype= np.uint32)
                
                npPairing_GPU = cuda.mem_alloc(npPairing.nbytes)
                resultGenDeg_GPU = cuda.mem_alloc(resultGenDeg.nbytes)
                
                cuda.memcpy_htod(npPairing_GPU,npPairing)        
                cuda.memcpy_htod(resultGenDeg_GPU,resultGenDeg)                          
                cuda.memcpy_htod(numANDsConst,numOfAND)     
                cuda.memcpy_htod(XCons,block)
            
                start.record()
                generalCountAND(bitarray_GPU,resultGenDeg_GPU,npPairing_GPU,block=(max_threads,1,1),grid=((num_terms/max_threads)+1,1))              
                end.record()
                end.synchronize()
                secs = start.time_till(end)*1e-3
                print("3.1 algo",secs)
                cuda.memcpy_dtoh(resultGenDeg,resultGenDeg_GPU)
               # print(resultGenDeg)
                hw[i]=resultGenDeg
    ...
    def calculate_and_obs_counts(self,top_comb_current, num_top_terms, poly, termLen, polyLen, numOfPolys):
        """
        top_comb_curr - 1tice alebo 2jice
        num_top_terms cislo top terms
        poly polynomy
        termLen how many indices have term
        polyLen how many terms are there in one polynom
        numOfPolys number of all polynoms
        """  
        #print("pocitame AND")
        #freeTotal = cuda.mem_get_info()
        #print("free:", freeTotal[0]/(1024*1024),"MB","total:",freeTotal[1]/(1024*1024),"MB")        
        #print("prvy prvok",poly[0])
        #pre kazdy &, potom ak polyLen > 1 tak aj XOR pre kazdy
        
        #CPU alloc
        npPoly = np.array(poly,dtype=np.int32,order='C')
        result = np.zeros(numOfPolys,dtype=np.uint32,order='C')
        alignment = adjust_size64(self.base[0].length())
        varsInBlock = np.uint32((self.base[0].length() + alignment) / 64)        
        
        #GPU alloc
        npPoly_GPU = cuda.mem_alloc(npPoly.nbytes)
        result_GPU = cuda.mem_alloc(result.nbytes)
        
        #CPU to GPU transfer
        cuda.memcpy_htod(npPoly_GPU, npPoly)
        cuda.memcpy_htod(result_GPU,result)
        
        max_threads = self.get_max_threads()
        
        if top_comb_current == 1:
            mergedBArr = bitarray.bitarray()
            for i in range(0,len(self.base)):
                mergedBArr += self.base[i]
                mergedBArr += bitarray.bitarray('0' * alignment)
            
            self.subresult_AND = np.zeros(numOfPolys * varsInBlock,dtype= np.int64)
            
            #might be problematic for some PCs since this bitarray is n*12800*64 bits long for top n polynoms, datasize = 100MB
            bitarray_GPU = cuda.mem_alloc(mergedBArr.length()/8)
            subresult_GPU = cuda.mem_alloc(self.subresult_AND.nbytes)            
            
            cuda.memcpy_htod(bitarray_GPU, mergedBArr)                     
            cuda.memcpy_htod(subresult_GPU,self.subresult_AND)
     
            #freeTotal = cuda.mem_get_info()
            #print("free:", freeTotal[0]/(1024*1024),"MB","total:",freeTotal[1]/(1024*1024),"MB")                        
            calculateAndTermsInPoly(bitarray_GPU,subresult_GPU,result_GPU, npPoly_GPU, termLen, (numOfPolys*polyLen), varsInBlock,block=(max_threads,1,1),grid=(((numOfPolys*polyLen)/max_threads)+1,1))
            
            #GPU to CPU transfer 
            #subresult for next computations
            cuda.memcpy_dtoh(self.subresult_AND,subresult_GPU)
            cuda.memcpy_dtoh(result,result_GPU)
            
        if top_comb_current >= 2:
            polyLen = np.int32(top_comb_current) #because for AND its all in one list [1,2,3,4,5,6] = [1,2,3] & [4,5,6], so we get wrong values otherwise 
            combs = generateCombinations(num_top_terms,polyLen)
            combinations = np.array(combs,dtype= np.int32)
            
            subresult_GPU = cuda.mem_alloc(self.subresult_AND.nbytes)
            combinations_GPU = cuda.mem_alloc(combinations.nbytes)
            
            cuda.memcpy_htod(subresult_GPU,self.subresult_AND)
            cuda.memcpy_htod(combinations_GPU,combinations)
            
            #freeTotal = cuda.mem_get_info()
            #print("free:", freeTotal[0]/(1024*1024),"MB","total:",freeTotal[1]/(1024*1024),"MB")                        
            calculateANDcombinations(subresult_GPU,combinations_GPU,result_GPU,polyLen,numOfPolys,varsInBlock, block=(max_threads,1,1),grid=((numOfPolys/max_threads)+1,1))
            cuda.memcpy_dtoh(result,result_GPU)
        #freeTotal = cuda.mem_get_info()
        #print("free:", freeTotal[0]/(1024*1024),"MB","total:",freeTotal[1]/(1024*1024),"MB")                    
        return result        
    
    def calculate_xor_obs_counts(self,top_comb_current, num_top_terms, poly, termLen, polyLen, numOfPolys):
        """
        top_comb_curr - 1tice alebo 2jice
        num_top_terms cislo top terms
        poly polynomy
        ...
        npPoly = np.array(poly,dtype=np.int32,order='C')
        alignment = adjust_size64(self.base[0].length())
        varsInBlock = np.int32((self.base[0].length() + alignment) / 64)  
        #print("result velkost:",numOfPolys)
        result = np.zeros(numOfPolys,dtype=np.uint32,order='C')
        ...
        if top_comb_current == 1:
            ...
            self.subresult_XOR = np.zeros(numOfPolys * varsInBlock,dtype= np.int64)
               
            bitarray_GPU = cuda.mem_alloc(mergedBArr.length()/8)
            subresult_GPU = cuda.mem_alloc(self.subresult_XOR.nbytes)            
            
            cuda.memcpy_htod(bitarray_GPU, mergedBArr)                     
            cuda.memcpy_htod(subresult_GPU,self.subresult_XOR)
            
            #freeTotal = cuda.mem_get_info()
            #print("free:", freeTotal[0]/(1024*1024),"MB","total:",freeTotal[1]/(1024*1024),"MB")
            
            #print(termLen, (numOfPolys*polyLen), varsInBlock)
            #print("bitarray:",mergedBArr.length()/8, "subres:", self.subresult_XOR.nbytes, "npPoly:", npPoly.nbytes)
            calculateAndTermsInPoly(bitarray_GPU,subresult_GPU,result_GPU, npPoly_GPU, termLen, (numOfPolys*polyLen), varsInBlock,block=(max_threads,1,1),grid=(((numOfPolys*polyLen)/max_threads)+1,1))
            
            cuda.memcpy_dtoh(self.subresult_XOR,subresult_GPU)
            cuda.memcpy_dtoh(result,result_GPU)
        ...
        if top_comb_current >= 2:
            ...
            #print("combinacii je", len(combs), "lebo numtopterms je", num_top_terms, "polyLen je", polyLen)            
            #print("top comb 2")
            #freeTotal = cuda.mem_get_info()
            #print("free:", freeTotal[0]/(1024*1024),"MB","total:",freeTotal[1]/(1024*1024),"MB")
            #print(polyLen, numOfPolys, varsInBlock)
            calculateXORcombinations(subresult_GPU,combinations_GPU,result_GPU,polyLen,numOfPolys,varsInBlock, block=(max_threads,1,1),grid=((numOfPolys/max_threads)+1,1))
